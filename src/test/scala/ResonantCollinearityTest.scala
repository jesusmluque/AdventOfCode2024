...
class ResonantCollinearityTest extends AnyFlatSpec:
  ...
  "The total numbers of antinode in the example1 file " should " be 14 " in:
    assert(ResonantCollinearity.countAntinodesIn(Source.fromResource("ResonantCollinearityExample1.txt").getLines().toList) == 14)
  "The total numbers of antinode in the exercise file " should " be 259 " in :
    assert(ResonantCollinearity.countAntinodesIn(Source.fromResource("ResonantCollinearityExercise.txt").getLines().toList) == 259)
